...
export function SurveyWorkspace() {
  ...
  const tab = (to: string, label: string, icon: ReactNode) => (
    <NavLink
      to={to}
      className={({ isActive }) =>
        `py-2 px-4 text-xs font-bold rounded-xl flex items-center gap-2 whitespace-nowrap ${
          isActive ? 'text-white bg-slate-900' : 'text-slate-600 hover:bg-slate-100'
        }`
      }
    >
      {icon}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1">
        {tab('edit', 'Pola i treść', <FileEdit className="w-4 h-4" />)}
      ...
      <Outlet key={survey.id} context={{ survey, reload }} />
    </div>
  );
}
